...
def run_eval(model, eval_loader, args, output_suffix=''):
  ...
  for i, (batch, original_batch, ctx_frames, filename) in enumerate(eval_loader):

      original_batch = Variable(original_batch.cuda(), volatile=True)

      original, out_imgs, losses, code_batch = eval_forward(
          model, (original_batch, ctx_frames, filename), args)

      losses, msssim, psnr = finish_batch(
          args, filename, original, out_imgs, 
          losses, code_batch, output_suffix)
