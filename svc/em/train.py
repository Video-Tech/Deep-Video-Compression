...
old_encoder = torch.load("./../bm/model/wunet_2:256_3:256_64x16_encoder_" + str(args.max_train_iters))
old_binarizer = torch.load("./../bm/model/wunet_2:256_3:256_64x16_binarizer_" + str(args.max_train_iters))

while True:

    for batch, (crops, crops_original, ctx_frames, temp) in enumerate(train_loader):
        scheduler.step()
        train_iter += 1
	...
	#print(enc_unet_output2[0].shape, unet_output1[0].shape, warped_unet_output1[0].shape)
	    ...
	    #print(old_codes.shape, codes.shape, new_codes.shape)
            ...
            losses.append(original.abs().mean() + loss_D)
