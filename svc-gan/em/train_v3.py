...
old_encoder = torch.load("./../bm/model/wunet_encoder_30000")
old_binarizer = torch.load("./../bm/model/wunet_binarizer_3000")
    ...
    for batch, (crops, ctx_frames, _) in enumerate(train_loader):
        ...
        if is_dis_training == True: ####################### DISCRIMINATOR
          ...
          for _ in range(args.iterations):
              ...
              # Binarize.
              new_codes = binarizer(encoded)

              old_encoded, old_encoder_h_1, old_encoder_h_2, old_encoder_h_3 = old_encoder(encoder_input, old_encoder_h_1, old_encoder_h_2, old_encoder_h_3, warped_unet_output1, warped_unet_output2)
              old_codes = old_binarizer(old_encoded)
              codes = torch.cat([new_codes, old_codes], dim=1)
          ...
          if just_resumed or train_iter % args.eval_iters == 0 or train_iter == 1000:
              ...
              torch.save(encoder, '{}wunet_encoder_{}'.format('./model/', train_iter) )
              torch.save(binarizer, '{}wunet_binarizer_{}'.format('./model/', train_iter) )
              torch.save(decoder, '{}wunet_decoder_{}'.format('./model/', train_iter) )
