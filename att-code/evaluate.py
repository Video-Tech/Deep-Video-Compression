...
def finish_batch(args, filenames, original, out_imgs,
  ...
  all_losses, all_msssim, all_psnr, all_att_msssim, all_att_psnr = [], [], [], [], []
  for ex_idx, filename in enumerate(filenames):
      ...
      msssim, psnr, attmsssim, attpsnr= evaluate(
...
def run_eval(model, eval_loader, args, output_suffix=''):
  ...
  for i, (batch, ctx_frames, filenames) in enumerate(eval_loader):
      ...
      losses, msssim, psnr, att_msssim, att_psnr = finish_batch(
      ...
      all_msssim += msssim
      all_psnr += psnr
      all_att_msssim += att_msssim
      all_att_psnr += att_psnr
  ...
  return (np.array(all_losses).mean(axis=0),
          ...
          np.array(all_att_msssim).mean(axis=0),
          np.array(all_att_psnr).mean(axis=0))
