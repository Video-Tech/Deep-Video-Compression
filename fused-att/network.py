...
class EncoderCell(nn.Module):
    def __init__(self, v_compress, stack, fuse_encoder, fuse_level):
        ...
        self.rnn1 = ConvLSTMCell(
            65,#128 if fuse_encoder and v_compress else 64,
            128,
            kernel_size=3,
            stride=2,
            padding=1,
    ...
    def forward(self, input, hidden1, hidden2, hidden3,
                unet_output1, unet_output2, gm):

        x = self.conv(input)
        #print(x.shape, unet_output1[2].shape, unet_output1[1].shape, gm[0].shape)
        # Fuse
        #if self.v_compress and self.fuse_encoder:
        #    x = torch.cat([x, unet_output1[2], unet_output2[2]], dim=1)
        gm0 = torch.from_numpy(gm[0]).float().cuda()
        x = torch.cat([x, gm0], dim=1)
...
class DecoderCell(nn.Module):
    def __init__(self, v_compress, shrink, bits, fuse_level):
        ...
        self.rnn4 = ConvLSTMCell(
            33,#(48 + 48//shrink*2) if v_compress else 32, #out3=64
            128,
            kernel_size=3,
            stride=1,
            padding=1,
